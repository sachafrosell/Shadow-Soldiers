...
public class RocketControllerP2 : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (whom == "player")
        {
            ...
            if (collision.gameObject.tag != "Player2" && collision.gameObject.tag != "Bullet" && collision.gameObject.tag != "Cieling" && collision.gameObject.tag != "Target2")
            {
                Destroy(gameObject);
                Destroy(trail);
        ...
        else
        {
            ...
            if (collision.gameObject.tag != "Enemy" && collision.gameObject.tag != "Bullet" && collision.gameObject.tag != "Cieling" && collision.gameObject.tag != "Target2")
            {
                Destroy(gameObject);
                Destroy(trail);
                big = Instantiate(bigExplosionSound, rb.position, Quaternion.identity);
