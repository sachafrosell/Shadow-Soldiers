        ...
        if (whom == "player")
        {
            if (collision.gameObject.tag == "Bird" || collision.gameObject.tag == "Enemy")
            {
                ...
                if (collision.gameObject.tag == "Enemy")
                {
                    if (singlePlayer) 
                    { 
                        Instantiate(enemy, new Vector3(rand, 10f), Quaternion.identity);
                    }
            ...
            if (collision.gameObject.tag != "Player" && collision.gameObject.tag != "Bullet" && collision.gameObject.tag != "Cieling" && collision.gameObject.tag != "Target")
            {
                Destroy(gameObject);
                Destroy(trail);
